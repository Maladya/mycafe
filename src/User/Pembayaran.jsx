 ...
 *   { kode_promo, tipe_diskon, nilai, nama_promo, minimum_order, mulai_date, berakhir_date }
 *   { code, discount_type, discount_value, description, min_order }
 *   { code, discount, description, minOrder }
 * → selalu { code, discountLabel, discountType, discountValue, description, minOrder, nama_promo, startDate, endDate }
 */
function normalisePromo(raw) {
  // sudah dinormalise sebelumnya
  if (raw?.discountLabel) return raw;

  // Handle format dari KelolaPromo.jsx (snake_case dari database)
  ...
  // Handle tipe diskon: 0/persen/percent = percent, 1/nominal/fixed = fixed
  let discountType = "";
  ...
  return { code, discountLabel, discountType, discountValue, description, minOrder, nama_promo, startDate, endDate };
}

const todayStr = () => new Date().toISOString().split("T")[0];

const isPromoActive = (p) => {
  if (!p.startDate || !p.endDate) return true; // Jika tidak ada tanggal, anggap aktif
              ...
              {orderedItems.map(item => (
                <div key={item.id} className="flex items-center gap-3 px-4 py-3">
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <p className="text-sm font-bold" style={{ color: "var(--p)" }}>Rp{(cart[item.id] * item.price).toLocaleString()}</p>
                  </div>
                </div>
              ))}
            </div>
