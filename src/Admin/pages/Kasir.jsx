...
export default function Kasir() {
  ...
  useEffect(() => {
    ...
    const fetchCafeSettings = async () => {
      ...
      try {
        ...
        const res = await fetch(`${API_URL}/api/pengaturan/user/${cafeId}`, { headers: authHeaders() });
  ...
  const handleLogout = () => {
    ...
    ["token", "user", "refresh_token", "cafe"].forEach(k => localStorage.removeItem(k));
...
function QRScanModal({ onClose, onScan }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => startCamera(), 500); // Delay 500ms untuk DOM siap
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm px-4">
      ...
      <div className="bg-gray-900 rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl">
        ...
        <div className="relative bg-black" style={{ minHeight: 300 }}>
          ...
          {/* div#qr-reader HARUS selalu ada di DOM dengan ukuran tetap */}
